...
def record(request):
    if request.method == "POST":
        ...
        if os.path.exists('C:/AgriChatbot/ChatBotApp/static/record.wav'):
            os.remove('C:/AgriChatbot/ChatBotApp/static/record.wav')
        if os.path.exists('C:/AgriChatbot/ChatBotApp/static/record1.wav'):
            os.remove('C:/AgriChatbot/ChatBotApp/static/record1.wav')    
        fs.save('C:/AgriChatbot/ChatBotApp/static/record.wav', audio_data)
        path = 'C:/AgriChatbot/ChatBotApp/static/'
        res = subprocess.check_output(path+'ffmpeg.exe -i '+path+'record.wav '+path+'record1.wav', shell=True)
        with sr.WavFile('C:/AgriChatbot/ChatBotApp/static/record1.wav') as source:
            audio = recognizer.record(source)
        try:
            text = recognizer.recognize_google(audio)
        except Exception as ex:
            text = "I am not trained to answer given question"    
        if os.path.exists('C:/AgriChatbot/ChatBotApp/static/record.csv'):
            os.remove('C:/AgriChatbot/ChatBotApp/static/record.csv')    
        fout = open('C:/AgriChatbot/ChatBotApp/static/record.csv', 'wb')
        fout.write(text.encode("utf-8"))
        fout.close()
        testArray = []
        with open('C:/AgriChatbot/ChatBotApp/static/record.csv', "r") as f:
            lines = f.readlines()
            for line in lines:
                line = line.strip("\n").strip()
...
def UploadAction(request):
    if request.method == 'POST':
        ...
        fs = FileSystemStorage()
        if os.path.exists('C:/AgriChatbot/ChatBotApp/static/plant/test.png'):
            os.remove('C:/AgriChatbot/ChatBotApp/static/plant/test.png')
        filename = fs.save('C:/AgriChatbot/ChatBotApp/static/plant/test.png', myfile)
        ...
        img = cv2.imread('C:/AgriChatbot/ChatBotApp/static/plant/test.png')
